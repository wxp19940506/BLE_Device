package com.example.xiaopengwang.ble_device.service;
...
public class BluetoothLeService extends Service {
	...
	private final BluetoothGattCallback mGattCallback = new BluetoothGattCallback()
	{
		...
		public String getEncoding(byte[] bytes) {
			...
			try {
				if (bytes.equals(new String(bytes, encode).getBytes(encode))) {
					String s = encode;
					return s;
				}
...
//				Log.e("encode_tag",getEncoding(sucString));
				Log.e("encode_tag",sucString+"");
                String string = null;

                string = new String(sucString);

                Log.e("DATATAG",string);
...
//			Log.e("data_length",data.length+"");
            ...
            for (byte byteChar : data)
			{
                ...
                dataList.add(String.format("%02X ", byteChar));
				Log.e(TAG, "***broadcastUpdate: byteChar = " + byteChar);

			}
			Log.i(TAG, "***broadcastUpdate: data = " + data);
			Log.i(TAG, "size = " + dataList.size());

//			intent.putExtra(EXTRA_DATA, new String(data));
//			intent.putExtra(EXTRA_DATA,data);

			intent.putStringArrayListExtra(EXTRA_DATA, (ArrayList<String>) dataList);
